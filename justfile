...
deploy:
    sudo nixos-rebuild --accept-flake-config switch --flake .
...
# Build and deploy this NixOS derivation
test:
    sudo nixos-rebuild --accept-flake-config test --flake .

# Print debug info while attempting to build and deploy this NixOS derivation
debug:
    sudo nixos-rebuild --accept-flake-config build --flake . --show-trace --verbose --option eval-cache false

# Update all flake inputs (nixCats-nvim first, then main flake)
update:
    cd /home/bruno/git/nixCats-nvim && just update
    sudo nix flake update --accept-flake-config

# Update a specific flake input
up input:
    sudo nix flake update {{ input }} --accept-flake-config
